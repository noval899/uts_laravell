...
class PostController extends Controller
{
    ...
    */
    ...
    {
        $datakantins = datakantin::latest()->when(request()->search, function($datakantins) {
            $datakantins = $datakantins->where('namakantin', 'like', '%'. request()->search . '%');
        })->paginate(5);
    ...
    */
    public function update(Request $request, datakantin $post)
    {
        ...
        if($request->file('image') == "") {
            ...
            //hapus old image
            Storage::disk('local')->delete('public/datakantins/'.$post->image);
            
            //upload new image
            $image = $request->file('image');
            $image->storeAs('public/datakantins', $image->hashName());
